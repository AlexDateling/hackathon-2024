...
function chaincode_command_group() {
  ...
  if [ "${COMMAND}" == "deploy" ]; then
    ...
    log "Installing chaincode for absa"
    install_chaincode $@
    log "🏁 - Chaincode is installed"

  elif [ "${COMMAND}" == "approve" ]; then
    log "Approving chaincode for absa"
    approve_chaincode $@
    log "🏁 - Chaincode is approved"

  elif [ "${COMMAND}" == "commit" ]; then
    log "Committing chaincode for absa"
    commit_chaincode $@
...
function query_chaincode() {
  ...
  export_peer_context absa peer1
...
function query_chaincode_metadata() {
  ...
  log ''
  log 'SARB-Peer2:'
  export_peer_context absa peer2
  peer chaincode query -n $cc_name -C $CHANNEL_NAME -c $args
}
...
function invoke_chaincode() {
  ...
  peer chaincode invoke \
    ...
    -C              $CHANNEL_NAME \
    -c              $@ \
    --orderer       sarb-orderer1.${DOMAIN}:${NGINX_HTTPS_PORT} \
    --connTimeout   ${ORDERER_TIMEOUT} \
    --tls --cafile  ${TEMP_DIR}/channel-msp/ordererOrganizations/sarb/orderers/sarb-orderer1/tls/signcerts/tls-cert.pem \
    ${INVOKE_EXTRA_ARGS}
