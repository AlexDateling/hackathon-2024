...
function register_org_admins() {
  ...
  register_org_admin sarb sarbadmin sarbadminpw
  register_org_admin absa absaadmin absaadminpw
  register_org_admin crossborder crossborderadmin crossborderadminpw
...
function enroll_org_admins() {
  ...
  enroll_org_admin orderer  sarb sarbadmin sarbadminpw
  enroll_org_admin peer     absa absaadmin absaadminpw
  enroll_org_admin peer     crossborder crossborderadmin crossborderadminpw
...
function create_channel_MSP() {
  ...
  extract_orderer_tls_cert sarb orderer3
...
function join_channel_orderers() {
  ...
  join_channel_orderer sarb orderer1
  join_channel_orderer sarb orderer2
  join_channel_orderer sarb orderer3
...
function join_channel_peer() {
  ...
  peer channel join \
    ...
    --orderer     sarb-orderer1.${DOMAIN} \
    --connTimeout ${ORDERER_TIMEOUT} \
    --tls         \
    --cafile      ${TEMP_DIR}/channel-msp/ordererOrganizations/sarb/orderers/sarb-orderer1/tls/signcerts/tls-cert.pem
}
